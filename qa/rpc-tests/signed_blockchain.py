...
class SignedBlockchainTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        print("Testing blocksigning of various sizes with 2 signing nodes:")
        max_keys = 20
        for num_keys in [x for x in range(max_keys+1) if x % 4 == 0 and x >= 8]:
            print("Signing with " + str(num_keys) + " keys")
            if num_keys > 16:
                push_num_keys = "01"+str(hex(num_keys))[2:]
            else:
                push_num_keys = str(hex(80+num_keys)[2:])
            push_key = "21"
            CMS = "ae"
            big_multisig = push_num_keys
            for i in range(num_keys//2):
                addr = self.nodes[0].getnewaddress()
                big_multisig += push_key+self.nodes[0].validateaddress(addr)["pubkey"]
            for i in range(num_keys//2+num_keys%2):
                addr = self.nodes[1].getnewaddress()
                big_multisig += push_key+self.nodes[1].validateaddress(addr)["pubkey"]

            big_multisig += push_num_keys+CMS

            signblockscript="-signblockscript="+big_multisig

            stop_nodes(self.nodes)
            # reindex to wipe genesis block and have new signblockscript
            self.nodes = start_nodes(3, self.options.tmpdir, [["-reindex", signblockscript],["-reindex", signblockscript],["-reindex", signblockscript]])
            new_block = self.nodes[0].getnewblockhex()
            sig0 = self.nodes[0].signblock(new_block)
            sig1 = self.nodes[1].signblock(new_block)
            assert(sig0 != "00")
            assert(sig1 != "00")
            result = self.nodes[0].combineblocksigs(new_block, [sig0, sig1])
            assert(result["complete"])
            block_height = self.nodes[0].getblockcount()
            self.nodes[0].submitblock(result["hex"])
            self.nodes[1].submitblock(result["hex"])
            self.nodes[2].submitblock(result["hex"])
