...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    for (const auto& txout : tx.vout)
    {
        if (!txout.nValue.IsValid())
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, "bad-txns-vout-amount-invalid");
    ...
    if (tx.IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < tx.vout.size(); i++) {
            if (tx.vout[i].IsFee()) {
                return state.Invalid(ValidationInvalidReason::CONSENSUS, false, "bad-cb-fee");
            }
        }
    }
    else
