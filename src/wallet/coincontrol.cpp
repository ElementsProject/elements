...
// ELEMENTS
void PreselectedInput::SetPeginWitness(const CScriptWitness& script_wit)
{
    m_pegin_witness = script_wit;
}

std::optional<CScriptWitness> PreselectedInput::GetPeginWitness() const
{
    return m_pegin_witness;
}

std::optional<CScriptWitness> CCoinControl::GetPeginWitness(const COutPoint& outpoint) const
{
    const auto it = m_selected.find(outpoint);
    return it != m_selected.end() ? m_selected.at(outpoint).GetPeginWitness() : std::nullopt;
}
} // namespace wallet
