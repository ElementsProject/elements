...
UniValue getethpeginaddress(const JSONRPCRequest& request)
{
    ...
    AuditLogPrintf("%s : getethpeginaddress %s\n", getUser(), pubkey.GetHash().GetHex());

    Witnessifier w;
    CTxDestination dest = vchAddress;
    bool ret = boost::apply_visitor(w, dest);
    if (!ret) throw JSONRPCError(RPC_WALLET_ERROR, "Public key or redeemscript not known to wallet, or the key is uncompressed");
    pwalletMain->SetAddressBook(w.result, "", "receive");

    CScript witScript = GetScriptForDestination(dest);
    CScript witProg = GetScriptForWitness(witScript);
    std::string destAddress = Params().GetConsensus().fedpegAddress.ToString();

    AuditLogPrintf("%s : getethpeginaddress eth_mainchain_address: %s eth_claim_script: %s\n",
        getUser(), destAddress, HexStr(witProg));

    UniValue fundinginfo(UniValue::VOBJ);
    fundinginfo.pushKV("eth_mainchain_address", destAddress);
    fundinginfo.pushKV("eth_claim_script", HexStr(witProg));
    return fundinginfo;
}
...
UniValue validateethpegin(const JSONRPCRequest& request)
{
    ...
    if (!IsValidEthPegin(tx, nAmount, strFailReason) && IsConfirmedEthPegin(tx, strFailReason)) {
...
UniValue createrawethpegin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "1. \"txid\"            (hex, required) Eth transaction peg-in transaction id\n"
            "2. \"amount\"          (numeric, required) Eth transaction peg-in amount\n"
            "3. \"claim_script\"    (string, optional) The witness program generated by getethpeginaddress\n"
            "Result:\n"
    ...
    // Witness script corresponding to eth address for signing the peg-in
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "claimethpegin",            &claimethpegin,             true,   {"txid", "amount", "claim_script"}},
    { "wallet",             "createrawethpegin",        &createrawethpegin,         true,   {"txid", "amount", "claim_script"}},
