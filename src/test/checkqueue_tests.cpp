    ...
    for (size_t i = 0; i < 1001; ++i) {
        ...
        while (remaining) {
            ...
            for (size_t k = 0; k < r && remaining; k++, remaining--) {
                vChecks.emplace_back(new FixedCheck(remaining == 1 ? std::make_optional<int>(17 * i) : std::nullopt));
            }
            control.Add(std::move(vChecks));
        }
    ...
    for (auto times = 0; times < 10; ++times) {
        for (const bool end_fails : {true, false}) {
            CCheckQueueControl<FixedCheck> control(fail_queue.get());
            {
                ...
                vChecks.reserve(100);
                for (int i = 0; i < 100; ++i) {
                    vChecks.push_back(new FixedCheck(std::nullopt));
                }
                delete vChecks[99];
                vChecks[99] = new FixedCheck(end_fails ? std::make_optional<int>(2) : std::nullopt);
                control.Add(std::move(vChecks));
            }
