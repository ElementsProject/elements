...
-- 'fastEval' always returns the same result including when evaluating 'disconnect' (assuming the jetType is faithful).
-- This result is different from substituing jets which changes the commitment roots, which in turn changes the result of evaluating 'disconnect'.
