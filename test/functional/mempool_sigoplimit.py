...
class BytesPerSigOpTest(BitcoinTestFramework):
    ...
    def test_legacy_sigops_stdness(self):
        ...
        total_in = 1_000 * len(outpoints)
        nonstd_tx = CTransaction()
        nonstd_tx.vin = [CTxIn(op, CScript([b"", packed_redeem_script])) for op in outpoints]
        nonstd_tx.vout = [CTxOut(0, CScript([OP_RETURN, b""])), CTxOut(total_in, CScript())] # ELEMENTS: explicit fee output
        ...
        std_tx.vin.pop()
        std_tx.vout[1].nValue.setToAmount(total_in - 1_000) # ELEMENTS: balance fee
