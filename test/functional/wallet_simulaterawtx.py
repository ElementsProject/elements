...
class SimulateTxTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(w0.getbalance()["bitcoin"], 50.0)
        assert_equal(w1.getbalance()["bitcoin"], 0.0)
        ...
        assert_equal(w0.simulaterawtransaction([tx1])["balance_change"]["bitcoin"], 0.0)
        assert_equal(w2.simulaterawtransaction([tx1])["balance_change"]["bitcoin"], 5.0)

        # w1 should see +5 balance for tx1
        assert_equal(w1.simulaterawtransaction([tx1])["balance_change"]["bitcoin"], 5.0)

        # w0 should be unaffected, w2 should see +5 for both transactions
        assert_equal(w0.simulaterawtransaction([tx1, tx2])["balance_change"]["bitcoin"], 0.0)
        assert_equal(w2.simulaterawtransaction([tx1, tx2])["balance_change"]["bitcoin"], 5.0)

        # w1 should see +15 balance for both transactions
        assert_equal(w1.simulaterawtransaction([tx1, tx2])["balance_change"]["bitcoin"], 15.0)
        ...
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"]["bitcoin"], -(Decimal("5") + bitcoin_fee))
        assert_approx(w2.simulaterawtransaction([tx1])["balance_change"]["bitcoin"], Decimal("5"))
        ...
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"]["bitcoin"], -Decimal("5") - bitcoin_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"]["bitcoin"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"]["bitcoin"], -Decimal("5") - bitcoin_fee)
        assert_approx(w1.simulaterawtransaction([tx1, tx4])["balance_change"]["bitcoin"], Decimal("4.9999"))
        ...
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"]["bitcoin"], -(Decimal("10") + bitcoin_fee2))
        assert_approx(w1.simulaterawtransaction([tx2])["balance_change"]["bitcoin"], +(Decimal("10")))
        assert_approx(w2.simulaterawtransaction([tx2])["balance_change"]["bitcoin"], 0)
        ...
        # ELEMENTS: blinded tx
        addr = w1.getnewaddress("", "blech32")
        tx = w0.createrawtransaction([], [{addr: 5.0}])
        tx_fund = w0.fundrawtransaction(tx)
        fee = tx_fund["fee"]
        tx_blind = w0.blindrawtransaction(tx_fund["hex"], False)
        assert_approx(w0.simulaterawtransaction([tx_blind])["balance_change"]["bitcoin"], -(Decimal("5") + fee))
        assert_approx(w1.simulaterawtransaction([tx_blind])["balance_change"]["bitcoin"], +(Decimal("5")))

        # ELEMENTS: issued asset
        issued = w0.issueasset(100, 1)
        asset = issued["asset"]
        addr1 = w1.getnewaddress("", "blech32")
        addr2 = w1.getnewaddress("", "blech32")
        tx = w0.createrawtransaction([], [{addr1: 5.0}, {addr2: 50, "asset": asset}])
        tx_fund = w0.fundrawtransaction(tx)
        fee = tx_fund["fee"]
        tx_blind = w0.blindrawtransaction(tx_fund["hex"], False)
        sim1 = w0.simulaterawtransaction([tx_blind])
        sim2 = w1.simulaterawtransaction([tx_blind])
        assert_approx(sim1["balance_change"]["bitcoin"], -(Decimal("5") + fee))
        assert_approx(sim1["balance_change"][asset], -Decimal("50"))
        assert_approx(sim2["balance_change"]["bitcoin"], +Decimal("5"))
