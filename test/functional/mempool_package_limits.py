...
class MempoolPackageLimitsTest(BitcoinTestFramework):
    ...
    def test_desc_count_limits(self):
        ...
        fee = 0.0002
        parent_value = (first_coin["amount"] - Decimal(str(fee))) / 2 # Deduct reasonable fee and make 2 outputs
        inputs = [{"txid": first_coin["txid"], "vout": 0}]
        outputs = [{self.address : parent_value}, {ADDRESS_BCRT1_P2WSH_OP_TRUE : parent_value}, {"fee": fee}]
        ...
        # ELEMENTS: add fee output
        fee = 0.0001
        value = parent_value - Decimal(str(fee))
        rawtx_b = node.createrawtransaction([{"txid": parent_txid, "vout": 1}], [{self.address : value}, {"fee": fee}])
    ...
    def test_desc_count_limits_2(self):
        ...
        # ELEMENTS: add fee output
        fee = 0.0002
        parent_value = (first_coin_a["amount"] - Decimal(str(fee))) / 2 # Deduct reasonable fee and make 2 outputs
        ...
        # P1
        value_p1 = (parent_value - Decimal(str(fee)))
        rawtx_p1 = node.createrawtransaction([{"txid": parent_txid, "vout": 1}], [{self.address : value_p1}, {"fee": fee}])
