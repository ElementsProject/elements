...
class RPCPackagesTest(BitcoinTestFramework):
    ...
    def test_submit_cpfp(self):
        ...
        tx_rich = self.wallet.create_self_transfer(fee=Decimal('0.00000001'), fee_rate=0)
        node.prioritisetransaction(tx_rich["txid"], 0, int(DEFAULT_FEE * COIN))
        package_txns = [tx_rich, tx_poor]
        coins = [tx["new_utxo"] for tx in package_txns]
        tx_child = self.wallet.create_self_transfer_multi(utxos_to_spend=coins, fee=10000) # ELEMENTS
        ...
        elements_fee = Decimal('0.00000001')
        assert_equal(rich_parent_result["fees"]["base"], elements_fee) # ELEMENTS
        assert_equal(poor_parent_result["fees"]["base"], elements_fee) # ELEMENTS
        assert_equal(child_result["fees"]["base"], DEFAULT_FEE)
        # The "rich" parent does not require CPFP so its effective feerate.
        assert_fee_amount(DEFAULT_FEE + elements_fee, tx_rich["tx"].get_vsize(), rich_parent_result["fees"]["effective-feerate"])
        ...
        assert_fee_amount(DEFAULT_FEE + elements_fee, tx_poor["tx"].get_vsize() + tx_child["tx"].get_vsize(), poor_parent_result["fees"]["effective-feerate"])
        assert_fee_amount(DEFAULT_FEE + elements_fee, tx_poor["tx"].get_vsize() + tx_child["tx"].get_vsize(), child_result["fees"]["effective-feerate"])
