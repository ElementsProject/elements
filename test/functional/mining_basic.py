...
class MiningTest(BitcoinTestFramework):
    ...
    def test_block_max_weight(self):
        ...
        # ELEMENTS: Due to Elements' signed block overhead (300 weight units added to
        # DEFAULT_BLOCK_RESERVED_WEIGHT), LARGE_VSIZE calculation involves int truncation
        # that prevents exact fill to MAX_BLOCK_WEIGHT. Use approximate check instead.
        mempool_weight = self.nodes[0].getmempoolinfo()['bytes'] * WITNESS_SCALE_FACTOR
        self.log.info(f"Testing that the mempool's weight approximately matches the maximum block weight: {mempool_weight} ~= {MAX_BLOCK_WEIGHT}.")
        assert_greater_than_or_equal(MAX_BLOCK_WEIGHT, mempool_weight)
        assert_greater_than_or_equal(mempool_weight, MAX_BLOCK_WEIGHT - 1000)
