...
class WalletGroupTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        with self.nodes[3].assert_debug_log(['Fee non-grouped = 5140, grouped = 6520, using grouped']):
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        with self.nodes[3].assert_debug_log(['Fee non-grouped = 7880, grouped = 10620, using non-grouped']):
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        with self.nodes[4].assert_debug_log(['Fee non-grouped = 7880, grouped = 10620, using grouped']):
