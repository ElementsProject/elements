...
class WalletAnchorTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        ...
        # ELEMENTS: Pay-to-anchor outputs carry a 0-value, spendable scriptPubKey.
        # Elements consensus (`VerifyAmounts` in confidential_validation.cpp) rejects
        # any 0-value output whose scriptPubKey is not `IsUnspendable()`, so the P2A
        # construction used by this test cannot be broadcast on an Elements chain.
        raise SkipTest("Pay-to-anchor outputs are rejected by Elements consensus (0-value spendable output)")
