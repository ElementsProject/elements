...
class TapHashPeginTest(BitcoinTestFramework):
    ...
    def tapscript_satisfy_test(self, script, inputs = [], add_issuance = False,
        ...
        if fail:
            assert_raises_rpc_error(-26, None, self.nodes[0].sendrawtransaction, tx.serialize().hex())
            return
    ...
    def run_test(self):
        ...
        self.tapscript_satisfy_test(CScript([OP_0, OP_0, OP_INSPECTINPUT, b'\x00', OP_EQUAL]))
        # 1b. Add a pegin (Test pegin input must be 0x40)
        self.tapscript_satisfy_test(CScript([OP_0, OP_0, OP_INSPECTINPUT, b'\x40', OP_EQUAL]), add_pegin=True)
        # 1c. Add peing (The non-pegin input must be push 0x00)
        self.tapscript_satisfy_test(CScript([OP_1, OP_0, OP_INSPECTINPUT, b'\x00', OP_EQUAL]), add_pegin=True)
        # 1d. Add an issuance
        self.tapscript_satisfy_test(CScript([OP_0, OP_0, OP_INSPECTINPUT, b'\x80', OP_EQUAL]), add_issuance=True)
        # 1e. Both issuance and pegins together
        self.tapscript_satisfy_test(CScript([OP_0, OP_0, OP_INSPECTINPUT, b'\xc0', OP_EQUAL]), add_pegin = True, add_issuance=True)
        ...
        self.tapscript_satisfy_test(CScript([OP_0, OP_2, OP_INSPECTINPUT, 1, OP_EQUALVERIFY, BITCOIN_ASSET_BYTES, OP_EQUAL]))
        for _ in range(5): # run multiple times for wallet to select conf utxos
            self.tapscript_satisfy_test(CScript([OP_1, OP_2, OP_INSPECTINPUT, OP_TOALTSTACK, OP_EQUALVERIFY, OP_FROMALTSTACK, OP_EQUAL]), blind=True, add_asset=True)

        # 3 Test asset(explicit and conf)
        amt = 12*10**7
        self.tapscript_satisfy_test(CScript([OP_0, OP_3, OP_INSPECTINPUT, 1, OP_EQUALVERIFY, amt.to_bytes(8, 'little'), OP_EQUAL]))
        for _ in range(5): # run multiple times for wallet to select conf utxos
            self.tapscript_satisfy_test(CScript([OP_1, OP_3, OP_INSPECTINPUT, OP_TOALTSTACK, OP_EQUALVERIFY, OP_FROMALTSTACK, OP_EQUAL]), blind=True, add_value=True)

        # 4 Test scriptpubkey
        self.tapscript_satisfy_test(CScript([OP_0, OP_4, OP_INSPECTINPUT, OP_EQUAL]), add_spk = True)

        # 5 Test nSequence
        one, two = 1, 2
        self.tapscript_satisfy_test(CScript([OP_0, OP_5, OP_INSPECTINPUT, one.to_bytes(4, 'little'), OP_EQUAL]), seq = 1)
        self.tapscript_satisfy_test(CScript([OP_0, OP_5, OP_INSPECTINPUT, two.to_bytes(4, 'little'), OP_EQUAL]), seq = 1, fail=True)

        # 6 Test issuances
        # No issaunce
        self.tapscript_satisfy_test(CScript([OP_0, OP_6, OP_INSPECTINPUT, OP_FALSE, OP_EQUAL]))
        # Issuance with 2 units
        asset_issue_amount = 2*COIN
        self.tapscript_satisfy_test(CScript([
            OP_0, OP_6, OP_INSPECTINPUT,
